...
- One follow-up item remains in `TODO.md`, fully specified with approved design decisions and input-free: create/rename/delete of projects and roles directly in the `/workflows` project and role menus via single-key hotkeys (`n`/`r`/`d`) acting on the hovered item, with the workflow layer unchanged (check/uncheck toggle list) and CRUD touching only `projects.json` ids/assignments (never workflow `.md` files). The `managing_roles` frontmatter field removal is complete; the proposed Ctrl+S save-and-exit feature was withdrawn in favor of the existing Escape save-before-exit behavior.
