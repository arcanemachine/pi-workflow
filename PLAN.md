...
- One follow-up item remains in `TODO.md`, fully specified with approved design decisions and needing no new input: a Ctrl+S save-and-exit confirmation (reuse the existing atomic save, available at every menu level, in-scope `\x13` interception in each `ctx.ui.custom` `handleInput`). The `managing_roles` frontmatter field removal is complete.
