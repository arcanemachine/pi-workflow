...
- Two follow-up items remain in `TODO.md`: (1) remove the display-only `managing_roles` frontmatter field entirely (fully specified, authorized, needs no new input); (2) a Ctrl+S save-and-exit confirmation (blocked on three open design questions and an explicit "do not implement until the user approves" note in `TODO.md`).
