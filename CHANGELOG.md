...
- Make semantic and operational `pi_workflow` failures reject as Pi tool errors, preserving specific error codes and visible collapsed recovery text instead of rendering successful result rows.
- Clarify configured project IDs, explicit global-workflow permission, the no-fallback boundary, and `list` versus `list_global` parameter behavior.
